    ...
    with open(txt_path, 'w', encoding='utf-8') as f:
        for key, value in dictionary.items():
            ...
            if paddle.is_tensor(value):
                value_str = f"Tensor(shape={value.shape}, dtype={value.dtype})"
            ...
            if key == "position_ids":
                tmp_value = value.squeeze(0)
                length = tmp_value.shape[0]
                num_chunk = length // 100
                remiander = length % 100
                for i in range(num_chunk):
                    f.write(f"{tmp_value[i*100:(i+1)*100, :]} \n")
                f.write(f"{tmp_value[num_chunk*100:, :]} \n")
...
def load_npz_to_paddle(file_path="/root/paddlejob/workspace/env_run/laipeiwen/code4git/ERNIE/alignment/swift/input.npz"):
    ...
    for key, value in data.items():
        if isinstance(value, np.ndarray):
            paddle_data[key] = paddle.to_tensor(value)
        else:
            paddle_data[key] = paddle.to_tensor(np.array(value))
    return paddle_data